...
def gold_xp(fig, data):
    ...
    # plot画折线图，x轴为gold和xp索引，此处为比赛时间，纵轴为gold_chang和xp差值，表示带数据点
    gold, = ax1.plot(gold_change.index, gold_change.values, color="b")
    ...
    ax1.text(-6, (list(ax1.axes.get_ylim())[1])/2, str(data['radiant_team']['tag']),
             rotation=90, fontsize=13, color='k')  # 添加文字说明，前两个表示文字坐标，rotation旋转角度
    ax1.text(-6, (list(ax1.axes.get_ylim())
                   [0])/2, str(data['dire_team']['tag']), rotation=90, fontsize=13, color='k')
    ax1.spines['bottom'].set_position(('data', 0))  # 设置x轴的到数据为0的位置
    ax1.spines['left'].set_position(('data', 0))  # 设置y轴的到数据为0的位置
    ax1.set_title("{} vs {} 经济、经验差".format(
        data['radiant_team']['tag'], data['dire_team']['tag']))  # 设置标题
    ax1.legend(handles=[gold, xp], labels=[
               "经济", "经验"], loc='upper left')  # 设置图例


# 输出
def damages_gold(fig, data):
    laye = {i['name']: round(i['hero_damage']/i['total_gold'], 2)
...
# 选手金钱变化
def player_gold_change(fig, data):
    ...
    for i in range(10):
        locals()['p{}'.format(i)], = ax4.plot(gold_t.index,
                                              gold_t[gold_t.columns[i]])  # 批量命名变量用到locals函数
        lines.append(locals()['p{}'.format(i)])

    ax4.set_title("{} vs {} 选手金钱变化".format(
        data['radiant_team']['tag'], data['dire_team']['tag']))  # 设置标题
    ax4.legend(handles=lines, labels=list(
        gold_t.columns), loc='upper left')  # 设置图例


def main():
    match_id = input("请输入职业比赛id:\n")
    data = requests.get(
        "https://api.opendota.com/api/matches/"+str(match_id)).json()
    fig = plt.figure(figsize=(14, 7))  # 新建图像，figsize为图像大小
    gold_xp(fig, data)
    damages_gold(fig, data)
    total_damages(fig, data)
    player_gold_change(fig, data)
    fig.suptitle("{} vs {} 比赛部分数据对比".format(
        data['radiant_team']['tag'], data['dire_team']['tag']))
